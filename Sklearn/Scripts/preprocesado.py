...
def concatenar_estados(dataframe):
	...
	df = pd.concat([dataframe,dfAnterior,dfAnterior2],axis=1)
	df.fillna(999)
	return df
...
newNames = ["("+str(x)+","+str(y)+")" for x,y in itertools.product(np.linspace(-2,2,5), np.linspace(0,10,5))]
...
#print(train_data.ix[10])
#print(target_data.ix[10])
